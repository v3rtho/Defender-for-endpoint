...
function Get-IsDownLevelServer {
    ...
    # Server 2016
    if ($ver.Major -eq 10 -and $ver.Minor -eq 0 -and $ver.Build -eq 14393) {
        return $true
    }
...
try {
    ...
    if ($action -eq 'install') {
        ...
        elseif ($osVersion.Major -eq 10 -and $osVersion.Minor -eq 0 -and $osVersion.Build -lt 18362) {
            ...
            Trace-Message "Skipped updates for modern Windows Servers"
        }
    }
    ...
    Trace-Message "IsDownLevelServer: $IsDownLevelServer"

    if ($IsDownLevelServer) {
    $exitCode = Measure-Process -FilePath:$((Get-Command 'msiexec.exe').Path) -ArgumentList:$argumentList -PassThru
        ...
        Trace-Message "Skipping md4ws.msi installation (not a downlevel server)."
        Trace-Message "$action successful."
    }
